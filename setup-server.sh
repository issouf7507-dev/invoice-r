...
sudo mkdir -p /var/www/webapp/facturer/{current,backups,temp,logs}
sudo chown -R $USER:$USER /var/www/webapp/facturer

# Rendre le script de déploiement exécutable
echo "🔧 Configuration des permissions..."
chmod +x /var/www/webapp/facturer/deploy.sh
...
sudo tee /etc/nginx/sites-available/facturer << EOF
server {
    listen 80;
    server_name facture.royalcargo225.com; # Remplacez par votre domaine
...
# Activer le site
sudo ln -s /etc/nginx/sites-available/facturer /etc/nginx/sites-enabled/
sudo nginx -t
